...
﻿
(function(){if(window.CKEDITOR&&window.CKEDITOR.dom)return;if(!window.CKEDITOR){window.CKEDITOR=(function(){var basePathSrcPattern=/(^|.*[\\\/])ckeditor\.js(?:\?.*|;.*)?$/i;var CKEDITOR={timestamp:'GBLA',version:'4.6.1',revision:'0',rnd:Math.floor(Math.random()*(999-100+1))+100,_:{pending:[],basePathSrcPattern:basePathSrcPattern},status:'unloaded',basePath:(function(){var path=window.CKEDITOR_BASEPATH||'';if(!path){var scripts=document.getElementsByTagName('script');for(var i=0;i<scripts.length;i++){var match=scripts[i].src.match(basePathSrcPattern);if(match){path=match[1];break;}}}
...
console[type](prefix+'For more information about this error go to http://docs.ckeditor.com/#!/guide/dev_errors-section-'+errorCode);},null,null,999);CKEDITOR.dom={};(function(){var functions=[],cssVendorPrefix=CKEDITOR.env.gecko?'-moz-':CKEDITOR.env.webkit?'-webkit-':CKEDITOR.env.ie?'-ms-':'',ampRegex=/&/g,gtRegex=/>/g,ltRegex=/</g,quoteRegex=/"/g,tokenCharset='abcdefghijklmnopqrstuvwxyz0123456789',TOKEN_COOKIE_NAME='ckCsrfToken',TOKEN_LENGTH=40,allEscRegex=/&(lt|gt|amp|quot|nbsp|shy|#\d{1,5});/g,namedEntities={lt:'<',gt:'>',amp:'&',quot:'"',nbsp:'\u00a0',shy:'\u00ad'},allEscDecode=function(match,code){if(code[0]=='#'){return String.fromCharCode(parseInt(code.slice(1),10));}else{return namedEntities[code];}};CKEDITOR.on('reset',function(){functions=[];});CKEDITOR.tools={arrayCompare:function(arrayA,arrayB){if(!arrayA&&!arrayB)
...
return true;}
return false;},keystrokeToString:function(lang,keystroke){var special=keystroke&0xFF0000,key=keystroke&0x00FFFF,isMac=CKEDITOR.env.mac,CTRL=17,CMD=224,ALT=18,SHIFT=16,display=[],aria=[];if(special&CKEDITOR.CTRL){display.push(isMac?'⌘':lang[CTRL]);aria.push(isMac?lang[CMD]:lang[CTRL]);}
...
isStartGood=(curItem.getPosition(boundaries.startNode)&CKEDITOR.POSITION_FOLLOWING)||boundaries.startNode.equals(curItem);isEndGood=(curItem.getPosition(boundaries.endNode)&(CKEDITOR.POSITION_PRECEDING+CKEDITOR.POSITION_IS_CONTAINED));if(isStartGood&&isEndGood){ret.push(curItem);}}}
...
var widths=element.styles.border.match(/([\.\d]+\w+)/g)||['0px'];switch(widths.length){case 1:element.styles['border-width']=widths[0];break;case 2:mapStyles([0,1,0,1]);break;case 3:mapStyles([0,1,2,1]);break;case 4:mapStyles([0,1,2,3]);break;}
element.styles['border-style']=element.styles['border-style']||(element.styles.border.match(/(none|hidden|dotted|dashed|solid|double|groove|ridge|inset|outset|initial|inherit)/)||[])[0];if(!element.styles['border-style'])
delete element.styles['border-style'];delete element.styles.border;function mapStyles(map){element.styles['border-top-width']=widths[map[0]];element.styles['border-right-width']=widths[map[1]];element.styles['border-bottom-width']=widths[map[2]];element.styles['border-left-width']=widths[map[3]];}},listTypeToStyle:function(element){if(element.attributes.type){switch(element.attributes.type){case'a':element.styles['list-style-type']='lower-alpha';break;case'A':element.styles['list-style-type']='upper-alpha';break;case'i':element.styles['list-style-type']='lower-roman';break;case'I':element.styles['list-style-type']='upper-roman';break;case'1':element.styles['list-style-type']='decimal';break;default:element.styles['list-style-type']=element.attributes.type;}}},splitMarginShorthand:function(element){if(!element.styles.margin){return;}
var widths=element.styles.margin.match(/(\-?[\.\d]+\w+)/g)||['0px'];switch(widths.length){case 1:element.styles.margin=widths[0];break;case 2:mapStyles([0,1,0,1]);break;case 3:mapStyles([0,1,2,1]);break;case 4:mapStyles([0,1,2,3]);break;}
...
function createDocumentFragmentFromRanges(ranges,editable){var docFragment=new CKEDITOR.dom.documentFragment(),tableClone,currentRow,currentRowClone;for(var i=0;i<ranges.length;i++){var range=ranges[i],container=range.startContainer;if(container.getName&&container.getName()=='tr'){if(!tableClone){tableClone=container.getAscendant('table').clone();tableClone.append(container.getAscendant('tbody').clone());docFragment.append(tableClone);tableClone=tableClone.findOne('tbody');}
...
currentRowClone.append(range.cloneContents());}else{docFragment.append(range.cloneContents());}}
...
var docFragment=createDocumentFragmentFromRanges(ranges,editable);return toString?docFragment.getHtml():docFragment;},extractSelectedHtml:function(toString,removeEmptyBlock){var editable=this.editable(),ranges=this.getSelection().getRanges();if(!editable||ranges.length===0){return null;}
...
delete keystrokes[keystroke];}},getCommandKeystroke:function(command){var commandName=command.name,keystrokes=this.keystrokeHandler.keystrokes,key;if(command.fakeKeystroke){return command.fakeKeystroke;}
...
try{if(CKEDITOR.env.ie&&!(CKEDITOR.env.edge&&CKEDITOR.env.version>14)&&this.getDocument().equals(CKEDITOR.document)){this.$.setActive();}else{this.$.focus();}}catch(e){if(!CKEDITOR.env.ie)
throw e;}
...
function checkFillingCharSequenceNodeReady(editable){var fillingChar=editable.getCustomData('cke-fillingChar');if(fillingChar){if(fillingChar.getCustomData('ready')){removeFillingCharSequenceNode(editable);}else{fillingChar.setCustomData('ready',1);}}}
...
if(CKEDITOR.env.webkit){editable.attachListener(doc,'keydown',function(evt){var key=evt.data.getKey();switch(key){case 13:case 33:case 34:case 35:case 36:case 37:case 39:case 8:case 45:case 46:removeFillingCharSequenceNode(editable);}},null,null,-1);}
...
if(bookmarks.isFake){node=ranges[0].getEnclosedNode();if(!node||node.type!=CKEDITOR.NODE_ELEMENT){CKEDITOR.warn('selection-not-fake');bookmarks.isFake=0;}}
if(bookmarks.isFake)
this.fake(node);else
...
function onDialogCommit(evt){evt.removeListener();evt.cancel();dialogCommited=true;callback({type:dataType,dataValue:evt.data.dataValue,dataTransfer:evt.data.dataTransfer,method:'paste'});}
...
function addPasteListenersToEditable(){var editable=editor.editable();if(CKEDITOR.plugins.clipboard.isCustomCopyCutSupported){var initOnCopyCut=function(evt){if(!editor.readOnly||evt.name!='cut'){clipboard.initPasteDataTransfer(evt,editor);}
...
return success;}};}
function createPasteCmd(){return{canUndo:false,async:true,fakeKeystroke:CKEDITOR.CTRL+86,exec:function(editor,data){var cmd=this,fire=function(data,withBeforePaste){data&&firePasteEvents(editor,data,!!withBeforePaste);editor.fire('afterCommandExec',{name:'paste',command:cmd,returnValue:!!data});};if(typeof data=='string')
...
return undefined;}};})();(function(){var template='<a id="{id}"'+' class="cke_button cke_button__{name} cke_button_{state} {cls}"'+
...
var name=this.name||this.command,iconName=name;if(this.icon&&!(/\./).test(this.icon)){iconName=this.icon;this.icon=null;}
...
editor.fire('saveSnapshot');});if(config.colorButton_enableAutomatic!==false){output.push('<a class="cke_colorauto" _cke_focus=1 hidefocus=true'+' title="',lang.auto,'"'+' onclick="CKEDITOR.tools.callFunction(',clickFn,',null,\'',type,'\');return false;"'+' href="javascript:void(\'',lang.auto,'\')"'+' role="option" aria-posinset="1" aria-setsize="',total,'">'+'<table role="presentation" cellspacing=0 cellpadding=0 width="100%">'+'<tr>'+'<td>'+'<span class="cke_colorbox" id="',colorBoxId,'"></span>'+'</td>'+'<td colspan=7 align=center>',lang.auto,'</td>'+'</tr>'+'</table>'+'</a>');}
output.push('<table role="presentation" cellspacing=0 cellpadding=0 width="100%">');for(var i=0;i<colors.length;i++){if((i%8)===0)
...
if(moreColorsEnabled){output.push('</tr>'+'<tr>'+'<td colspan=8 align=center>'+'<a class="cke_colormore" _cke_focus=1 hidefocus=true'+' title="',lang.more,'"'+' onclick="CKEDITOR.tools.callFunction(',clickFn,',\'?\',\'',type,'\');return false;"'+' href="javascript:void(\'',lang.more,'\')"',' role="option" aria-posinset="',total,'" aria-setsize="',total,'">',lang.more,'</a>'+'</td>');}
...
return true;});editor.on('selectionChange',function(){var html=[],elementsList=elementsPath.list=[],namesList=[],filters=elementsPath.filters,isContentEditable=true,elementsChain=editor.elementPath().elements,name;for(var j=elementsChain.length;j--;){var element=elementsChain[j],ignore=0;if(element.data('cke-display-name'))
...
this.refresh();}
editor.on('activeFilterChange',updateState,this);editor.on('mode',updateState,this);editor.on('selectionChange',updateState,this);!this.readOnly&&editor.on('readOnly',updateState,this);var keyDownFn=CKEDITOR.tools.addFunction(function(ev,element){ev=new CKEDITOR.dom.event(ev);var keystroke=ev.getKeystroke();if(keystroke==40){editor.once('panelShow',function(evt){evt.data._.panel._.currentBlock.onKeyDown(40);});}
switch(keystroke){case 13:case 32:case 40:CKEDITOR.tools.callFunction(clickFn,element);break;default:instance.onkey(instance,keystroke);}
...
delete removed[s];return{set:set,removed:CKEDITOR.tools.objectKeys(removed)};},showDisplayTextForElement:function(element,editor){var undesiredElements={img:1,table:1,tbody:1,thead:1,tfoot:1,input:1,select:1,textarea:1};if(editor.widgets&&editor.widgets.focused){return false;}
return!element||!element.getName||!element.is(undesiredElements);}};CKEDITOR.unlinkCommand=function(){};CKEDITOR.unlinkCommand.prototype={exec:function(editor){var style=new CKEDITOR.style({element:'a',type:CKEDITOR.STYLE_INLINE,alwaysRemoveElement:1});editor.removeStyle(style);},refresh:function(editor,path){var element=path.lastElement&&path.lastElement.getAscendant('a',true);if(element&&element.getName()=='a'&&element.getAttribute('href')&&element.getChildCount())
...
(editor.plugins.dialogadvtab?'table'+editor.plugins.dialogadvtab.allowedContent():''),requiredContent:'table',contentTransformations:[['table{width}: sizeToStyle','table[width]: sizeToAttribute'],['td: splitBorderShorthand'],[{element:'table',right:function(element){if(element.styles){if(element.styles.border&&element.styles.border.match(/solid/)){element.attributes.border=1;}
...
if(CKEDITOR.env.ie&&!CKEDITOR.env.edge&&editor.enterMode!=CKEDITOR.ENTER_P){removeSuperfluousElement('p');}else if(CKEDITOR.env.edge&&editor.enterMode!=CKEDITOR.ENTER_DIV){removeSuperfluousElement('div');}
...
lockRetain=true;}},null,null,1);editable.attachListener(editable,'keyup',function(){var elements=doc.getElementsByTag(tagName);if(lockRetain){if(elements.count()==1&&!elements.getItem(0).getCustomData('retain')&&!elements.getItem(0).hasAttribute('data-cke-temp')){elements.getItem(0).remove(1);}
lockRetain=false;}});}}
